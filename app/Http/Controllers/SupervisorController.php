...
class SupervisorController extends Controller
{
    ...
    {
        $publicaciones= DB::table('tbl_vehiculos as publicaciones')
        ->where('publicaciones.bol_eliminado','=',0)
        ->where('publicaciones.status_admin','=',710 )
        //->where('publicaciones.lng_idoper', '=' , '')
        //->orderBy('id')
        // PERSONAS      
        ...
        // RESPONSABLE
        ->leftjoin('tbl_admins as oper', 'publicaciones.lng_idoper', '=', 'oper.id')
        ->select(
            // ID - Publicaciones
            'publicaciones.id',
        ...
        $responsables= DB::table('tbl_admins as responsables')
       	->select('id','name')
       	->where('lng_idrol', '=', 4)
       	->get();
        
        return view('supervisor.publicaciones',compact('publicaciones','responsables'))->with('page_title', 'Publicaciones');
    }
    ...
    public function update(Request $request)
    {
    	...
    	//echo "resp->".$request["responsable"];
    	//echo "<br>";
    	//echo "pub->".$request["publicacion"];
    	//echo $id = Auth::user()->id;
    	if(!empty($request['publicacion'])){
    		$publicaciones  = array_values($request['publicacion']);
    		echo $total_publicaciones = count($publicaciones);
    		
    		foreach($publicaciones as $key => $publicacion)
    		{
    			$publicacion = DB::update('update tbl_vehiculos set lng_idoper = '.$request['responsable'].', lng_idadmin = '.Auth::user()->id.' where id = '.$publicacion.';');
    		}
    		/*for ($i = 0; $i <= $total_publicaciones - 1; $i++)
    		{
    			//echo $i . "<br>";
    			$publicacion = DB::update('update tbl_vehiculos set lng_idoper = '.$request['responsable'].', lng_idadmin = '.Auth::user()->id.' where id = '.$request->publicacion[$i].';'     );
    		}*/
    	}
    	Session::flash('message', 'La publicación ha sido actualizada exitosamente');
        return Redirect::route('supervisor.publicaciones');
    }
    ...
    public function detalle($id)
    {
        ...
        //return "Hola";
        $idVehiculo = $id;
        return view('supervisor.detalle',['idVehiculo'=>$id,'vehiculo'=>$vehiculo,'imagenesVehiculo'=>$imagenesVehiculo,'detalleVehiculo'=>$detalleVehiculo])->with('page_title', 'Detalles');
    }
    /**
